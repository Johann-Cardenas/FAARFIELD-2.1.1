...
5. **Snapshot lock (`gDetailedReportData.PreserveEvaluationSnapshot`):** PCR's GL-iteration (`NewAdjustAnnDepart2017` → `NewAdjustGrossWeight2` → repeated `PCNLifeCalc`) calls `LeafDesignFlex()` over and over, and `LeafDesignFlex` opens with `gDetailedReportData.Clear()`. Without protection, that `Clear()` nulls `EvaluationAircraftDetails` (and the `UserInput*` fields the renderer reads), which is exactly why a higher-than-default GTW used to display the converged-MGW responses instead of the user-input ones. `Clear()` now skips `EvaluationAircraftDetails` whenever `PreserveEvaluationSnapshot = True`. `btnPCR_Click` sets the flag right after `PropagateUserInputToDetails` and clears it on every exit path (`finish1:` and the `Catch` block). `AircraftDetails` is still nulled by `Clear()` as before — the renderer reads the snapshot, not the live array, so subsequent PCR rounds populating fresh `clsAircraftDetail` instances never touch the captured user-input responses.
