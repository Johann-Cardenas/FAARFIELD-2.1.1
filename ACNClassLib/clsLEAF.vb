...
Friend Class clsLEAF
    Public gResponseType As clsLEAF.LEAFoptions
    ...
    Dim EvalX(,) As Double 'ikawa
    Dim EvalY(,) As Double 'ikawa
    'Public EvalX(,) As Double
    'Public EvalY(,) As Double
    ...
    'begin kairat replace tandem
    Dim TireXMax() As Double
    Dim TireYMax() As Double
    Dim TireXMin() As Double
    Dim TireYMin(), offsetMax() As Double
    Public Const NOFF As Integer = 41 ' Number of offsets for CDF calculations.- kairat
    Public Const NNodesLong As Short = 1800 ' Number of nodes along longitudinal direction -kairat
    Public Const OFFSETINC As Single = 10.0! ' Distance between offsets in inches.
    Public gTandemFnew As Boolean
    'end kairat replace tandem
    ...
    Dim AllResp(,) As clsLEAF.LEAFAllResponses
    ...
    Private Sub ComputeResponseCheck(ByRef ResponseType As clsLEAF.LEAFoptions, ByRef NACarg As Integer, ByRef LEAAircraft() As clsLEAF.LEAFACParms, ByRef LEAStructure As clsLEAF.LEAFStrParms, ByRef Response(,) As Double, ByRef AllResps(,) As clsLEAF.LEAFAllResponses)
    ...
    Public Sub ComputeResponse(ByRef ResponseType As clsLEAF.LEAFoptions, ByRef NACarg As Integer, ByRef LEAAircraft() As clsLEAF.LEAFACParms, ByRef LEAStructure As clsLEAF.LEAFStrParms, ByRef Response(,) As Double, ByRef AllResps(,) As clsLEAF.LEAFAllResponses)
        ...
        ElseIf LEAStructure.InterfaceParm(1) = 1 And LEAStructure.InterfaceParm(2) = 1 And
        ...
        ElseIf ResponseType = LEAFoptions.VerticalStrains Then

            ReDim AllResp(NAC, NEvalPointsMax)
            Call FindAllResponses(EvalLayer, ZEval)
            Call FindAllStresses(EvalLayer, ZEval)
            ReDim AllResps(NAC, NEvalPointsMax)
            'ikawa AllResps = VB6.CopyArray(AllResp)
            'AllResps = System.Array.Copy(AllResp, AllResps, lenght)
    ...
    Private Sub IntegrateHorizontalStress(ByRef EvalLayer As Integer, ByRef ZEval As Double, ByRef Response(,) As Double)
        ...
        For IAC = 1 To NAC
            ...
            For IEval = 1 To NEvalPoints(IAC)
                ...
                'added to compute the largest stress among all evaluation points by YGC 092613
                'If Not IEval = 1 Then 'commented ik
                '    Response(IAC, 1) = Math.Max(Response(IAC, 1), Response(IAC, IEval))
                '    Response(IAC, IEval) = 0
                'End If
                'add ended to compute the largest stress among all evaluation points by YGC 092613
...
FindConstantsPartInvertError:
    ...
    Sub FindConstants(ByRef GLAlphaByZ As Double, ByRef B() As Double, ByRef OShift(,) As Double, ByRef Started As Boolean, ByRef CallNo As Integer)
        ...
        If DesignType = "FlexOnRigid" Or
           DesignType = "NewRigid" Or
            DesignType = "UnbondOnRigid" Or
            DesignType = "PartBondOnRigid" Or
...
2:          K = I
    ...
    Private Sub FindAllStresses(ByRef EvalLayer As Integer, ByRef ZEval As Double)
    ...
    Private Sub FindConstantsPartInvert_modified(ByVal ALPHA As Double, ByVal B() As Double, ByVal OShift(,) As Double, ByVal BadCondition As Long)
        ...
        FileOpen(4, "TTT.TXT", OpenMode.Output, , , 1024) 'clsLEAF
        PrintLine(4, "FEDFAA")
...
FindConstantsPartInvertError:
    ...
    Public Sub ComputeResponse3(ByRef ResponseType As clsLEAF.LEAFoptions, ByRef NACarg As Integer, ByRef LEAAircraft() As clsLEAF.LEAFACParms, ByRef LEAStructure As clsLEAF.LEAFStrParms, ByRef Response(,) As Double,
            ByRef AllResps(,) As clsLEAF.LEAFAllResponses, ByVal Ymin As Double, ByVal Ymax As Double) ' kairat replace tandem
        ...
        Dim NEvalx, NEvaly, IEvaly, IEvalx As Integer ' kairat replaces tandem
        Dim ResponseMax As Double 'kairat replaces tandem
        ...
        If ResponseType = LEAFoptions.VerticalStrain Then
            'begin kairat replace tandem
            Call IntegrateZStrain2(EvalLayer, ZEval, Response) ' kairat
            ...
            For IAC = 1 To NAC
                ...
                For IEval = 1 To NEvalPoints(IAC)
                    '     Compression is -ve, strain for LEDFAA must be -ve.
                    If Response(IAC, IEval) < ResponseMax Then
                        ResponseMax = (Response(IAC, IEval))
                        offsetMax(IAC) = EvalX(IAC, IEval)
                    End If
                Next IEval
                For ITire = 1 To NTires(IAC)
                    If TireY(IAC, ITire) > TireYMax(IAC) Then TireYMax(IAC) = TireY(IAC, ITire)
                    If TireY(IAC, ITire) < TireYMin(IAC) Then TireYMin(IAC) = TireY(IAC, ITire)
                Next ITire

                TireYMin(IAC) = Ymin
                TireYMax(IAC) = Ymax

                TireYMin(IAC) = TireYMin(IAC) - 160
                TireYMax(IAC) = TireYMax(IAC) + 160
            Next IAC

            NEvalPointsMax = NNodesLong
            For IAC = 1 To NAC
                With LEAAircraft(IAC)
                    NEvalPoints(IAC) = NEvalPointsMax
                End With
            Next IAC
            ...
            For IAC = 1 To NAC 'find Eval
                For IEvaly = 1 To NNodesLong
                    EvalX(IAC, IEvaly) = offsetMax(IAC) ' mesh in transverse direction at each 10 inches - kairat
                    EvalY(IAC, IEvaly) = TireYMin(IAC) + IEvaly * (TireYMax(IAC) - TireYMin(IAC)) / NNodesLong
                Next IEvaly
            Next IAC
            ...
            Call IntegrateZStrain2(EvalLayer, ZEval, Response) ' kairat
            'Call IntegrateZStrainK1(EvalLayer, ZEval, Response) ' kairat replace tandem
            'end kairat replace tandem
            ...
            For IAC = 1 To -NAC
                ...
                Next IEval
            Next IAC
            ' other response types not needed in this sub and deleted
        End If

        For IEval = -NEvalPoints(1) To 1 Step -1 ' 1 To NEvalPoints(1)
            '    Debug.Print LPad(10, Format(EvalY(NAC, IEval), "0.000000")); ", ";
            System.Diagnostics.Debug.Write(LPad(13, Format(AllResp(NAC, IEval).StressX, "0.00000E+00")) & ", ")
            System.Diagnostics.Debug.Write(LPad(13, Format(AllResp(NAC, IEval).StressY, "0.00000E+00")) & ", ")
            System.Diagnostics.Debug.Write(LPad(13, Format(AllResp(NAC, IEval).StressZ, "0.00000E+00")) & ", ")
            System.Diagnostics.Debug.WriteLine(LPad(13, Format(AllResp(NAC, IEval).StrainZ, "0.00000E+00")))
        Next IEval
    ...
    Public Sub ComputeResponse2(ByRef ResponseType As clsLEAF.LEAFoptions, ByRef NACarg As Integer, ByRef LEAAircraft() As clsLEAF.LEAFACParms, ByRef LEAStructure As clsLEAF.LEAFStrParms, ByRef Response(,) As Double, ByRef AllResps(,) As clsLEAF.LEAFAllResponses) ' kairat replace tandem
        ...
        If ResponseType = LEAFoptions.VerticalStrain Then
            ...
            For IEvaly = 1 To NNodesLong
                gEvalX(1, IEvaly) = EvalX(1, IEvaly) 'ikawa
                gEvalY(1, IEvaly) = EvalY(1, IEvaly) 'ikawa
            Next IEvaly
    ...
    Private Sub IntegrateZStrain2(ByRef EvalLayer As Integer, ByRef ZEval As Double, ByRef StrainW(,) As Double) ' kairat replace tandem
        ...
        For IAC = 1 To NAC
            'StrainWmax(IAC) = 0  ' we do not need max of strain at this subroutine kairat       
            Factor = TirePress(IAC, 1) * TireRadius(IAC, 1)
            Factor = Factor * (1 + Poissons(EvalLayer)) / Youngs(EvalLayer)
            For IEval = 1 To NEvalPoints(IAC)
                StrainW(IAC, IEval) = (StrainW(IAC, IEval)) * Factor
    ...
    Public Sub ComputeResponseAsym(ByRef ResponseType As clsLEAF.LEAFoptions, ByRef NACarg As Integer, ByRef LEAAircraft() As clsLEAF.LEAFACParms, ByRef LEAStructure As clsLEAF.LEAFStrParms, ByRef Response(,) As Double, ByRef AllResps(,) As clsLEAF.LEAFAllResponses) ' use asymptotic decomposition of LEAF  - Kairat
        ...
        ElseIf ResponseType = LEAFoptions.VerticalDeflection Then
            ...
            Call IntegrateZDeflectionAsym(EvalLayer, ZEval, Response)

        ElseIf ResponseType = LEAFoptions.AllResponses Then

            ReDim AllResp(NAC, NEvalPointsMax)
            Call FindAllResponsesAsym(EvalLayer, ZEval)
            ReDim AllResps(NAC, NEvalPointsMax)
            'ikawa AllResps = VB6.CopyArray(AllResp)
            'AllResps = System.Array.Copy(AllResp, AllResps, lenght)
    ...
    Private Sub IntegrateHorizontalStressAsym(ByRef EvalLayer As Integer, ByRef ZEval As Double, ByRef Response(,) As Double) 'KAirat , asym extraction method
        ...
        Dim B() As Double
        Dim IEval, IAC, ITire, IG As Integer
        Dim IGbreak As Integer 'Kairat

        Dim CK, AK, BK, DK As Double
        Dim CKa, AKa, BKa, DKa As Double 'Kairat 11/9/2016
        Dim ao0, ao1, ao2, bo0, bo1, co0, co1, do0 As Double 'Kairat 11/9/2016
        Dim IConstants As Integer
        Dim ZLayer As Double
        'ik* Dim AlphaZ, Theta As Double
        Dim Poisx2 As Double
        ...
        'begin Kairat
        Dim StressRTIG01a, StressRTIG11a As Double
        Dim StressRTIG02a, StressRTIG12a As Double
        Dim StressRTIG01axJ1A, StressRTIG11axJ1A As Double
        Dim StressRTIG02axJ1A, StressRTIG12axJ1A As Double
        Dim StressRIa, StressTIa As Double
        'end Kairat

        Dim StressR(,,) As Double
        Dim StressT(,,) As Double
        Dim StressRa(,,) As Double 'KAirat
        Dim StressTa(,,) As Double 'KAirat
        Dim StressX(,) As Double
        Dim StressY(,) As Double
        Dim TauXY(,) As Double
        Static StressHmax() As Double
        ...
        Dim TauRZ3(,,) As Double
        'begin Kairat
        Dim TauRZIG01a As Double 'ik* , TauRZIG11 As Double
        Dim TauRZIG02a As Double 'ik* , TauRZIG12 As Double
        Dim TauRZIG01axJ1A As Double 'ik* , TauRZIG11xJ1A As Double
        Dim TauRZIG02axJ1A As Double 'ik*, TauRZIG12xJ1A As Double
        Dim TauRZIa As Double
        Dim TauRZ3a(,,) As Double
        'end Kairat
        ...
        Dim hA As Integer = 1 'Kairat
        Dim hB As Integer = 1 'Kairat
        Dim hC As Integer = 1 'Kairat
        Dim hD As Integer = 1 'Kairat
        Dim SmallEvalLayer As Boolean 'Kairat
        Dim ExpAlpha As Double 'Kairat
        ...
        ReDim StressR(NAC, NTiresMax, NEvalPointsMax)
        ReDim StressT(NAC, NTiresMax, NEvalPointsMax)
        ReDim StressRa(NAC, NTiresMax, NEvalPointsMax) 'Kairat
        ReDim StressTa(NAC, NTiresMax, NEvalPointsMax) 'Kairat
        ReDim StressX(NAC, NEvalPointsMax)
        ReDim StressY(NAC, NEvalPointsMax)
        ReDim TauXY(NAC, NEvalPointsMax)
        ReDim Converged(NAC, NTiresMax, NEvalPointsMax)
        ...
        If FindingAllResponses Then
            ReDim TauRZ3(NAC, NTiresMax, NEvalPointsMax)
            ReDim TauRZ3a(NAC, NTiresMax, NEvalPointsMax) 'Kairat
            ReDim TauXZ(NAC, NEvalPointsMax)
            ReDim TauYZ(NAC, NEvalPointsMax)
        End If
        ...
        ' begin Kairat 
        SmallEvalLayer = False ' Kairat
        Call SetOShiftsAsym(OShift, I, ZLayer, RMax, SmallEvalLayer)
        ' end Kairat 
        ...
        ' begin Kairat 
        If SmallEvalLayer Then GoTo IHStress
        ao0 = fa(EvalLayer, 0) : ao1 = fa(EvalLayer, 1) : ao2 = fa(EvalLayer, 2)
        bo0 = fb(EvalLayer, 0) : bo1 = fb(EvalLayer, 1)
        co0 = fc(EvalLayer, 0) : co1 = fc(EvalLayer, 1)
        do0 = fd(EvalLayer)
        'end Kairat
        ...
        For IG = 1 To GLNGauss
            ...
            GLWeightbyZ = GLWeight(IG) / Z1
            ' begin Kairat
            ExpAlpha = System.Math.Exp(2 * GLAlpha(IG) / Z1 * (HLayer(1) - HLayer(EvalLayer)))
            If AK <> 0 Then
                AKa = (ao0 + GLAlpha(IG) / Z1 * (ao1 + GLAlpha(IG) / Z1 * ao2)) * ExpAlpha
            Else
                AKa = 0
            End If
            If CK <> 0 Then
                CKa = (co0 + GLAlpha(IG) / Z1 * co1) * System.Math.Exp(2 * GLAlpha(IG) / Z1 * (HLayer(1) - HLayer(EvalLayer)))
                CKa = (co0 + GLAlpha(IG) / Z1 * co1) * ExpAlpha
            Else
                CKa = 0
            End If
            StressRTIG01a = Poisx2 * CKa * GLWeightbyZ
            'end Kairat
            StressRTIG01 = Poisx2 * CK * GLWeightbyZ
            If FindingAllResponses Then
                TauRZIG01 = (AK + CK * (Poisx2 + GLAlpha(IG) * ZLayer1)) * GLWeightbyZ
                TauRZIG01a = (AKa + CKa * (Poisx2 + GLAlpha(IG) * ZLayer1)) * GLWeightbyZ 'Kairat
            End If
            ResponseforConverge = Poisx2 * System.Math.Abs(CKa - CK)
            CK = CK * (1 + GLAlpha(IG) * ZLayer1)
            StressRTIG11 = (AK + CK) * GLWeightbyZ
            CKa = CKa * (1 + GLAlpha(IG) * ZLayer1) 'Kairat
            StressRTIG11a = (AKa + CKa) * GLWeightbyZ 'Kairat
            ResponseforConverge = (ResponseforConverge + System.Math.Abs(AK - AKa) + System.Math.Abs(CK - CKa)) * GLWeightbyZ 'Kairat
            '    End If
            Call FindConstants(GLAlpha(IG) / Z2, B, OShift, FirstTimeFlag2, 2)
            BK = B(IConstants + 2)
            DK = B(IConstants + 4)
            ' begin Kairat
            ExpAlpha = System.Math.Exp(2 * GLAlpha(IG) / Z2 * HLayer(1))
            If BK <> 0 Then
                BKa = (bo0 + GLAlpha(IG) / Z2 * bo1) * ExpAlpha
            Else
                BKa = 0
            End If
            If DK <> 0 Then
            ...
            GLWeightbyZ = GLWeight(IG) / Z2
            StressRTIG02 = Poisx2 * DK * GLWeightbyZ
            StressRTIG02a = Poisx2 * DKa * GLWeightbyZ 'Kairat
            If FindingAllResponses Then
                TauRZIG02 = (BK - DK * (Poisx2 - GLAlpha(IG) * ZLayer1)) * GLWeightbyZ
                TauRZIG02a = (BKa - DKa * (Poisx2 - GLAlpha(IG) * ZLayer1)) * GLWeightbyZ 'Kairat
            End If
            ResponseforConverge = ResponseforConverge + Poisx2 * System.Math.Abs(DK - DKa) * GLWeightbyZ 'Kairat
            DK = DK * (1 - GLAlpha(IG) * ZLayer2)
            StressRTIG12 = -(BK - DK) * GLWeightbyZ
            DKa = DKa * (1 - GLAlpha(IG) * ZLayer2) 'Kairat
            StressRTIG12a = -(BKa - DKa) * GLWeightbyZ 'Kairat
            ResponseforConverge = ResponseforConverge + (System.Math.Abs(BK - BKa) + System.Math.Abs(DK - DKa)) * GLWeightbyZ 'Kairat
            For IAC = 1 To NAC
                If Not ACConverged(IAC) Then
                    A2 = TireRadius(IAC, 1) ' Move down 1 loop for varying pressures.
                    A1 = A2 / Z1
                    ...
                    If EvalLayer < NLayers Then
                        ...
                        If FindingAllResponses Then TauRZIG01xJ1A = TauRZIG01 * J1AlphaA1
                        'begin Kairat
                        StressRTIG01axJ1A = StressRTIG01a * J1AlphaA1
                        StressRTIG11axJ1A = StressRTIG11a * J1AlphaA1
                        If FindingAllResponses Then TauRZIG01axJ1A = TauRZIG01a * J1AlphaA1
                        'end Kairat
                    End If
                    J1AlphaA2 = LoadFunction(GLAlpha(IG), A2)
                    '        J1AlphaA2 = bessj1(GLAlpha(IG) * A2)
                    ...
                    If FindingAllResponses Then TauRZIG02xJ1A = TauRZIG02 * J1AlphaA2
                    'begin Kairat
                    StressRTIG02axJ1A = StressRTIG02a * J1AlphaA2
                    StressRTIG12axJ1A = StressRTIG12a * J1AlphaA2
                    If FindingAllResponses Then TauRZIG02axJ1A = TauRZIG02a * J1AlphaA2
                    'end Kairat
                    For ITire = 1 To NTires(IAC)
                        For IEval = 1 To NEvalPoints(IAC)
                            R2 = Radius(IAC, ITire, IEval)
                            ...
                            StressTI = StressTI + J1AlphaR1 * StressRTIG11xJ1A + J1AlphaR2 * StressRTIG12xJ1A
                            'begin Kairat
                            StressRIa = J0AlphaR1 * StressRTIG01axJ1A + J0AlphaR2 * StressRTIG02axJ1A
                            StressTIa = StressRIa
                            StressRIa = StressRIa + (J0AlphaR1 - J1AlphaR1) * StressRTIG11axJ1A + (J0AlphaR2 - J1AlphaR2) * StressRTIG12axJ1A
                            StressTIa = StressTIa + J1AlphaR1 * StressRTIG11axJ1A + J1AlphaR2 * StressRTIG12axJ1A
                            'end Kairat
                            '           Radial and tangential stresses are principal stresses.
                            StressR(IAC, ITire, IEval) = StressR(IAC, ITire, IEval) + StressRI - StressRIa 'Kairat
                            StressT(IAC, ITire, IEval) = StressT(IAC, ITire, IEval) + StressTI - StressTIa 'Kairat
                            StressRa(IAC, ITire, IEval) = StressRa(IAC, ITire, IEval) + StressRIa 'Kairat
                            StressTa(IAC, ITire, IEval) = StressTa(IAC, ITire, IEval) + StressTIa 'Kairat

                            If FindingAllResponses Then
                                TauRZI = J1AlphaR1 * TauRZIG01xJ1A * GLAlpha(IG) * R1 + J1AlphaR2 * TauRZIG02xJ1A * GLAlpha(IG) * R2
                                TauRZIa = J1AlphaR1 * TauRZIG01axJ1A * GLAlpha(IG) * R1 + J1AlphaR2 * TauRZIG02axJ1A * GLAlpha(IG) * R2 'Kairat
                                TauRZ3(IAC, ITire, IEval) = TauRZ3(IAC, ITire, IEval) + TauRZI - TauRZIa 'Kairat
                                TauRZ3a(IAC, ITire, IEval) = TauRZ3a(IAC, ITire, IEval) + TauRZIa 'Kairat
                            End If
        ...
        'begin Kairat integrate  asym part

        NACConverged = 0 'reinitialize convergence criteria
        ReDim Converged(NAC, NTiresMax, NEvalPointsMax)
        ReDim NEvalsConverged(NAC, NTiresMax)
        ReDim NTiresConverged(NAC)
        ReDim ACConverged(NAC)
        ...
        For IG = IGbreak To GLNGauss
            ResponseforConverge = 0
            '    If EvalLayer < NLayers Then  ' ***
            GLWeightbyZ = GLWeight(IG) / Z1
            ' begin Kairat
            ExpAlpha = System.Math.Exp(2 * GLAlpha(IG) / Z1 * (HLayer(1) - HLayer(EvalLayer)))
            AKa = hA * (ao0 + GLAlpha(IG) / Z1 * (ao1 + GLAlpha(IG) / Z1 * ao2)) * ExpAlpha
            CKa = hC * (co0 + GLAlpha(IG) / Z1 * co1) * ExpAlpha
            ...
            If FindingAllResponses Then
                TauRZIG01a = (AKa + CKa * (Poisx2 + GLAlpha(IG) * ZLayer1)) * GLWeightbyZ 'Kairat
            End If
            ResponseforConverge = Poisx2 * System.Math.Abs(CKa)
            CKa = CKa * (1 + GLAlpha(IG) * ZLayer1) 'Kairat
            StressRTIG11a = (AKa + CKa) * GLWeightbyZ 'Kairat
            ResponseforConverge = (ResponseforConverge + System.Math.Abs(AKa) + System.Math.Abs(CKa)) * GLWeightbyZ 'Kairat
            '    End If
            ' begin Kairat
            ExpAlpha = System.Math.Exp(2 * GLAlpha(IG) / Z2 * HLayer(1))
            BKa = hB * (bo0 + GLAlpha(IG) / Z2 * bo1) * ExpAlpha
            DKa = hD * do0 * ExpAlpha
            'end Kairat
            ...
            If FindingAllResponses Then
                TauRZIG02a = (BKa - DKa * (Poisx2 - GLAlpha(IG) * ZLayer1)) * GLWeightbyZ 'Kairat
            End If
            ResponseforConverge = ResponseforConverge + Poisx2 * System.Math.Abs(DKa) * GLWeightbyZ 'Kairat
            DKa = DKa * (1 - GLAlpha(IG) * ZLayer2) 'Kairat
            StressRTIG12a = -(BKa - DKa) * GLWeightbyZ 'Kairat
            ResponseforConverge = ResponseforConverge + (System.Math.Abs(BKa) + System.Math.Abs(DKa)) * GLWeightbyZ 'Kairat
            For IAC = 1 To NAC
                If Not ACConverged(IAC) Then
                    A2 = TireRadius(IAC, 1) ' Move down 1 loop for varying pressures.
                    A1 = A2 / Z1
                    ...
                    For ITire = 1 To NTires(IAC)
                        For IEval = 1 To NEvalPoints(IAC)
                            ...
                            '           Need to converge on the largest stress
                            Temp = System.Math.Abs(StressRa(IAC, ITire, IEval))
                            If System.Math.Abs(StressTa(IAC, ITire, IEval)) > Temp Then Temp = System.Math.Abs(StressTa(IAC, ITire, IEval))
                            If Temp < 0.000000000001 Then Temp = 0.000000000001
        ...
        For IAC = 1 To NAC
            For ITire = 1 To NTires(IAC)
                For IEval = 1 To NEvalPoints(IAC)
                    StressR(IAC, ITire, IEval) = StressRa(IAC, ITire, IEval) + StressR(IAC, ITire, IEval) 'Kairat
                    StressT(IAC, ITire, IEval) = StressTa(IAC, ITire, IEval) + StressT(IAC, ITire, IEval)  'Kairat
                    If FindingAllResponses Then
                        TauRZ3(IAC, ITire, IEval) = TauRZ3a(IAC, ITire, IEval) + TauRZ3(IAC, ITire, IEval) 'Kairat
                    End If
                Next IEval
            Next ITire
        Next IAC
        GoTo konH
        'end Kairat integrate  asym part
...
konH:
    ...
    Private Sub IntegrateZStrainAsym(ByRef EvalLayer As Integer, ByRef ZEval As Double, ByRef StrainW(,) As Double)  ' Uses asymptotic decomposition of integral - Kairat
        ...
        Dim A1, R1, R2, A2 As Double
        Dim StrainWI As Double
        Dim StrainWIa As Double 'Kairat
        Dim StrainWa(,) As Double 'Kairat

        'ik* Dim StrainW1, StrainW2 As Double
        Dim StrainWIG1, StrainWIG2 As Double
        Dim StrainWIG1a, StrainWIG2a As Double ' Kairat
        Dim StrainWIGforConverge As Double
        Dim StrainWmax() As Double
        Dim J1AlphaA1, J1AlphaA2 As Double
        Dim J0AlphaR1, J0AlphaR2 As Double
        Dim J1AlphaA1a, J1AlphaA2a As Double 'Kairat
        Dim Converged(,,) As Boolean
        Dim NEvalsConverged(,) As Integer
        Dim NTiresConverged() As Integer
        Dim ACConverged() As Boolean
        ...
        ReDim StrainW(NAC, NEvalPointsMax)
        ReDim StrainWa(NAC, NEvalPointsMax) ' Kairat
        ReDim StrainWmax(NAC)
        ReDim Converged(NAC, NTiresMax, NEvalPointsMax)
        ReDim NEvalsConverged(NAC, NTiresMax)
        ReDim NTiresConverged(NAC)
        ...
        ' begin Kairat
        If SmallEvalLayer Then GoTo IZDstrain
        ao0 = fa(EvalLayer, 0) : ao1 = fa(EvalLayer, 1) : ao2 = fa(EvalLayer, 2)
        bo0 = fb(EvalLayer, 0) : bo1 = fb(EvalLayer, 1)
        co0 = fc(EvalLayer, 0) : co1 = fc(EvalLayer, 1)
        do0 = fd(EvalLayer)
        ...
        For IG = 1 To GLNGauss
            ...
            If EvalLayer < NLayers Then
                ...
                End If
                If CK <> 0 Then
                    CKa = (co0 + GLAlpha(IG) / Z1 * co1) * ExpAlpha * (2 - Poisx2 * 2 - AlphaZ)
                Else
                    CKa = 0
                End If
                StrainWIG1a = -(AKa - CKa) * GLWeight(IG) / GLAlpha(IG)
                'end Kairat
                StrainWIG1 = -(AK - CK) * GLWeight(IG) / Z1
                'StrainWIGforConverge = (System.Math.Abs(AK) + System.Math.Abs(CK)) * GLWeight(IG) / Z1
                StrainWIGforConverge = (System.Math.Abs(AK - AKa) + System.Math.Abs(CK - CKa)) * GLWeight(IG) / Z1 'Kairat
            End If
            Call FindConstants(GLAlpha(IG) / Z2, B, OShift, FirstTimeFlag2, 2)
            BK = B(IConstants + 2)
            AlphaZ = GLAlpha(IG) * ZLayer2
            ...
            End If
            If DK <> 0 Then
                DKa = do0 * ExpAlpha * (2 - Poisx2 * 2 + AlphaZ)
            Else
                DKa = 0
            End If
            StrainWIG2a = (BKa + DKa) * GLWeight(IG) / Z2
            'end Kairat

            StrainWIG2 = (BK + DK) * GLWeight(IG) / Z2
            'StrainWIGforConverge = StrainWIGforConverge + (System.Math.Abs(BK) + System.Math.Abs(DK)) * GLWeight(IG) / Z2
            StrainWIGforConverge = StrainWIGforConverge + (System.Math.Abs(BK - BKa) + System.Math.Abs(DK - DKa)) * GLWeight(IG) / Z2 'Kairat
            ...
            For IAC = 1 To NAC
                If Not ACConverged(IAC) Then
                    ...
                    If EvalLayer < NLayers Then
                        J1AlphaA1 = LoadFunction(GLAlpha(IG), A1) * StrainWIG1
                        J1AlphaA1a = LoadFunction(GLAlpha(IG), A1) * StrainWIG1a 'Kairat
                        '          J1AlphaA1 = bessj1(GLAlpha(IG) * A1) * StrainWIG1
                    End If
                    J1AlphaA2 = LoadFunction(GLAlpha(IG), A2) * StrainWIG2
                    J1AlphaA2a = LoadFunction(GLAlpha(IG), A2) * StrainWIG2a
                    '        J1AlphaA2 = bessj1(GLAlpha(IG) * A2) * StrainWIG2
                    For ITire = 1 To NTires(IAC)
                        For IEval = 1 To NEvalPoints(IAC)
                            R2 = Radius(IAC, ITire, IEval)
                            ...
                            J0AlphaR2 = bessj0(GLAlpha(IG) * R2)
                            StrainWI = J0AlphaR1 * J1AlphaA1 + J0AlphaR2 * J1AlphaA2
                            StrainWIa = J0AlphaR1 * J1AlphaA1a + J0AlphaR2 * J1AlphaA2a 'Kairat

                            'StrainW(IAC, IEval) = StrainW(IAC, IEval) + StrainWI
                            StrainW(IAC, IEval) = StrainW(IAC, IEval) + StrainWI - StrainWIa  'Kairat
                            StrainWa(IAC, IEval) = StrainWa(IAC, IEval) + StrainWIa  'Kairat
                            If System.Math.Abs(StrainWIGforConverge / StrainW(IAC, IEval)) < ConvergenceLimit Then
                                If Not Converged(IAC, ITire, IEval) Then
                                    NEvalsConverged(IAC, ITire) = NEvalsConverged(IAC, ITire) + 1
                                    If NEvalsConverged(IAC, ITire) = NEvalPoints(IAC) Then
        ...
        For IG = IGbreak To GLNGauss
            ...
            If EvalLayer < NLayers Then
                ...
                ExpAlpha = System.Math.Exp(2 * GLAlpha(IG) / Z1 * (HLayer(1) - HLayer(EvalLayer)))
                AKa = hA * (ao0 + GLAlpha(IG) / Z1 * (ao1 + GLAlpha(IG) / Z1 * ao2)) * ExpAlpha
                CKa = hC * (co0 + GLAlpha(IG) / Z1 * co1) * ExpAlpha * (2 - Poisx2 * 2 - AlphaZ)
                StrainWIG1a = -(AKa - CKa) * GLWeight(IG) / GLAlpha(IG)
                StrainWIGforConverge = (System.Math.Abs(AKa) + System.Math.Abs(CKa)) * GLWeight(IG) / Z1 'Kairat
            End If
            AlphaZ = GLAlpha(IG) * ZLayer2
            'begin Kairat
            ExpAlpha = System.Math.Exp(2 * GLAlpha(IG) / Z2 * HLayer(1))
            BKa = hB * (bo0 + GLAlpha(IG) / Z2 * bo1) * ExpAlpha
            DKa = hD * do0 * ExpAlpha * (2 - Poisx2 * 2 + AlphaZ)
            'end Kairat
            StrainWIG2a = (BKa + DKa) * GLWeight(IG) / Z2
            StrainWIGforConverge = StrainWIGforConverge + (System.Math.Abs(BKa) + System.Math.Abs(DKa)) * GLWeight(IG) / Z2 'Kairat
                ...
                If Not ACConverged(IAC) Then
                    ...
                    For ITire = 1 To NTires(IAC)
                        For IEval = 1 To NEvalPoints(IAC)
                            ...
                            StrainWIa = J0AlphaR1 * J1AlphaA1a + J0AlphaR2 * J1AlphaA2a 'Kairat
                            StrainWa(IAC, IEval) = StrainWa(IAC, IEval) + StrainWIa  'Kairat
                            'If System.Math.Abs(StrainWIGforConverge / StrainWa(IAC, IEval)) < ConvergenceLimit Then
                            '    If Not Converged(IAC, ITire, IEval) Then
                            '        NEvalsConverged(IAC, ITire) = NEvalsConverged(IAC, ITire) + 1
                            '        If NEvalsConverged(IAC, ITire) = NEvalPoints(IAC) Then
                            '            NTiresConverged(IAC) = NTiresConverged(IAC) + 1
        ...
        For IAC = 1 To NAC
            For IEval = 1 To NEvalPoints(IAC)
                StrainW(IAC, IEval) = StrainWa(IAC, IEval) + StrainW(IAC, IEval)
            Next IEval
        Next IAC
        GoTo konstrain
        'end Kairat integrate  asym part
...
konstrain:
    ...
    Private Sub IntegrateZDeflectionAsym(ByRef EvalLayer As Integer, ByRef ZEval As Double, ByRef DeflW(,) As Double) ' Uses asymptotic decomposition of integral - Kairat
        ...
        'ik* Dim DeflMax() As Double
        Dim DeflWI As Double
        Dim DeflWIa As Double 'Kairat
        Dim DeflWa(,) As Double 'Kairat
        'ik* Dim DeflW1, DeflW2 As Double
        Dim DeflWIG1, DeflWIG2 As Double
        Dim DeflWIG1a, DeflWIG2a As Double ' Kairat
        Dim DeflWIGforConverge As Double
        Dim J1AlphaA1, J1AlphaA2 As Double
        Dim J0AlphaR1, J0AlphaR2 As Double
        Dim J1AlphaA1a, J1AlphaA2a As Double 'Kairat
        ...
        ReDim DeflW(NAC, NEvalPointsMax)
        ReDim DeflWa(NAC, NEvalPointsMax) 'Kairat
        Dim DeflWmax(NAC) As Double
        ReDim Converged(NAC, NTiresMax, NEvalPointsMax)
        ReDim NEvalsConverged(NAC, NTiresMax)
        ReDim NTiresConverged(NAC)
        ...
        ' begin Kairat 11/9/2016

        If SmallEvalLayer Then GoTo IZD

        'FileOpen(18, "FofAlfa.txt", OpenMode.Append) ' any integrand depending on GLAlpha Kairat
        ...
        'Dim iStartTime, iSecondTime, iThirdTime, iSecondsElapsed, iSecondsElapsedAsym As Double
        'iStartTime = Timer 'Remember time when macro starts
        ...
        For IG = 1 To GLNGauss
            ...
            If EvalLayer < NLayers Then
                ...
                Else
                    CKa = 0
                End If
                DeflWIG1a = -(AKa - CKa) * GLWeight(IG) / GLAlpha(IG)
                'end Kairat
                DeflWIG1 = -(AK - CK) * GLWeight(IG) / GLAlpha(IG)
                'DeflWIGforConverge = (System.Math.Abs(AK) + System.Math.Abs(CK)) * GLWeight(IG) / GLAlpha(IG)
                DeflWIGforConverge = (System.Math.Abs(AK - AKa) + System.Math.Abs(CK - CKa)) * GLWeight(IG) / GLAlpha(IG) 'Kairat
            End If
            Call FindConstants(GLAlpha(IG) / Z2, B, OShift, FirstTimeFlag2, 2)
            BK = B(IConstants + 2)
            AlphaZ = GLAlpha(IG) * ZLayer2
            ...
            Else
                DKa = 0
            End If
            DeflWIG2a = -(BKa + DKa) * GLWeight(IG) / GLAlpha(IG)
            'end Kairat

            DeflWIG2 = -(BK + DK) * GLWeight(IG) / GLAlpha(IG)
            'DeflWIGforConverge = DeflWIGforConverge + (System.Math.Abs(BK) + System.Math.Abs(DK)) * GLWeight(IG) / GLAlpha(IG)
            DeflWIGforConverge = DeflWIGforConverge + (System.Math.Abs(BK - BKa) + System.Math.Abs(DK - DKa)) * GLWeight(IG) / GLAlpha(IG)  'Kairat

            'PrintLine(18, IG & " " & GLAlpha(IG) & " " & DK & " " & DKa) 'Kairat
            'PrintLine(18, IG & " " & GLAlpha(IG) & " " & DeflWIG1 & " " & DeflWIG1a) 'Kairat
            ...
            For IAC = 1 To NAC
                If Not ACConverged(IAC) Then
                    ...
                    If EvalLayer < NLayers Then
                        J1AlphaA1 = LoadFunction(GLAlpha(IG), A1) * DeflWIG1
                        J1AlphaA1a = LoadFunction(GLAlpha(IG), A1) * DeflWIG1a ' Kairat
                    End If
                    J1AlphaA2 = LoadFunction(GLAlpha(IG), A2) * DeflWIG2
                    J1AlphaA2a = LoadFunction(GLAlpha(IG), A2) * DeflWIG2a 'Kairat
                    For ITire = 1 To NTires(IAC)
                        For IEval = 1 To NEvalPoints(IAC)
                            R2 = Radius(IAC, ITire, IEval)
                            R1 = R2 / Z1
                            ...
                            J0AlphaR2 = bessj0(GLAlpha(IG) * R2)
                            DeflWI = J0AlphaR1 * J1AlphaA1 + J0AlphaR2 * J1AlphaA2
                            DeflWIa = J0AlphaR1 * J1AlphaA1a + J0AlphaR2 * J1AlphaA2a 'Kairat

                            'If IAC = 1 And IEval = 1 Then PrintLine(18, IG & " " & GLAlpha(IG) & " " & DeflWI & " " & DeflWIa) 'Kairat

                            'DeflW(IAC, IEval) = DeflW(IAC, IEval) + DeflWI
                            DeflW(IAC, IEval) = DeflW(IAC, IEval) + DeflWI - DeflWIa 'Kairat
                            DeflWa(IAC, IEval) = DeflWa(IAC, IEval) + DeflWIa 'Kairat
                            If System.Math.Abs(DeflWIGforConverge / (DeflW(IAC, IEval) + 1.0E-200)) < ConvergenceLimit Then
                                If Not Converged(IAC, ITire, IEval) Then
                                    NEvalsConverged(IAC, ITire) = NEvalsConverged(IAC, ITire) + 1
                                    If NEvalsConverged(IAC, ITire) = NEvalPoints(IAC) Then
        ...
        For IG = IGbreak To GLNGauss
            ...
            If EvalLayer < NLayers Then
                ...
                CKa = hC * (co0 + GLAlpha(IG) / Z1 * co1) * ExpAlpha * (2 - Poisx2 * 2 - AlphaZ)
                DeflWIG1a = -(AKa - CKa) * GLWeight(IG) / GLAlpha(IG)
                DeflWIGforConverge = (System.Math.Abs(AKa) + System.Math.Abs(CKa)) * GLWeight(IG) / GLAlpha(IG) 'Kairat
            End If
            ...
            'end Kairat
            DeflWIG2a = -(BKa + DKa) * GLWeight(IG) / GLAlpha(IG)
            DeflWIGforConverge = DeflWIGforConverge + (System.Math.Abs(BKa) + System.Math.Abs(DKa)) * GLWeight(IG) / GLAlpha(IG)  'Kairat
                ...
                If Not ACConverged(IAC) Then
                    ...
                    For ITire = 1 To NTires(IAC)
                        For IEval = 1 To NEvalPoints(IAC)
                            ...
                            If System.Math.Abs(DeflWIGforConverge / (DeflWa(IAC, IEval) + 1.0E-200)) < ConvergenceLimit Then
                                If Not Converged(IAC, ITire, IEval) Then
                                    NEvalsConverged(IAC, ITire) = NEvalsConverged(IAC, ITire) + 1
                                    If NEvalsConverged(IAC, ITire) = NEvalPoints(IAC) Then
                                        NTiresConverged(IAC) = NTiresConverged(IAC) + 1
        ...
        'iSecondsElapsed = System.Math.Round(iSecondTime - iStartTime, 4)  'Determine how many seconds code took to run
        'iSecondsElapsedAsym = System.Math.Round(iThirdTime - iSecondTime, 4)  'Determine how many seconds code took to run

        For IAC = 1 To NAC
            For IEval = 1 To NEvalPoints(IAC)
                DeflW(IAC, IEval) = DeflWa(IAC, IEval) + DeflW(IAC, IEval)
            Next IEval
        Next IAC
        GoTo kon
        'end Kairat integrate  asym part
...
kon:
    ...
    Private Sub SetOShiftsAsym(ByRef OShift(,) As Double, ByRef IL As Integer, ByRef ZLayer As Double, ByRef RMax As Double, ByRef SmallEvalLayer As Boolean)
    ...
    Private Sub FindAllResponsesAsym(ByRef EvalLayer As Integer, ByRef ZEval As Double) 'Kairat

        Dim EL, I, IEval, IPad As Integer
        Dim IAC, NEvalPointsMax As Integer
        Dim Response(,) As Double
        Dim rDeflCurrent(,), rDeflNew(,), rStrainCurrent(,), rStrainNew(,), rStressCurrent(,), rStressNew(,) As Double 'Kairat
        Dim TempD, StrainG As Double
        'ik* Dim YYW, ZZW, XXW As Double
        'ik* Dim XZW, YZW, XYW As Double
        'ik* Dim XSW, ZSW, YSW As Double
        ...
        For I = 1 To NLayers
            prod = prod * InterfaceParm(I)
        Next

        If prod = 1 And EL < 5 And EL > 1 Then
            Call IntegrateZDeflectionAsym(EL, ZEval, Response) 'Kairat
            Call IntegrateZStrainAsym(EL, ZEval, Response) 'Kairat
            Call IntegrateHorizontalStressAsym(EL, ZEval, Response) 'Kairat
        Else
            Call IntegrateZDeflection(EL, ZEval, Response)
            Call IntegrateZStrain(EL, ZEval, Response)
            Call IntegrateHorizontalStress(EL, ZEval, Response)
    ...
    Private Function fa(ByRef EvalLayer As Integer, ByRef deg As Integer) As Double 'Kairat

        fa = 0.0

        If EvalLayer = NLayers Then GoTo faEnd

        Dim Rr(NLayers - 1) As Double
        Dim I As Integer

        ReDim Rr(NLayers - 1)

        For I = 1 To NLayers - 1
            Rr(I) = (1 + Poissons(I + 1)) * Youngs(I) / (Youngs(I + 1) * (1 + Poissons(I)))
        Next

        Select Case EvalLayer
            Case 1
                Select Case deg
                    Case 0
                        fa = (-2 - 2 * (-2 + Rr(1)) * Poissons(1)) / (3 + Rr(1) - 4 * Poissons(1)) +
                        (2 * (-1 + Poissons(1))) / (-1 + Rr(1) * (-3 + 4 * Poissons(2)))
                    Case 1
                        fa = (-4 * HLayer(1) * (-1 + Rr(1)) * Poissons(1)) / (3 + Rr(1) - 4 * Poissons(1))
                    Case 2
                        fa = (-2 * HLayer(1) ^ 2 * (-1 + Rr(1))) / (3 + Rr(1) - 4 * Poissons(1))
                End Select
            Case 2
                Select Case deg
                    Case 0
                        fa = (4 * (-1 + Poissons(1)) * (-13 - 3 * Rr(1) + 10 * Rr(2) + 6 * Rr(1) * Rr(2) + 3 * Rr(2) ^ 2 - 3 * Rr(1) * Rr(2) ^ 2 +
                        18 * Poissons(1) - 12 * Rr(2) * Poissons(1) - 6 * Rr(2) ^ 2 * Poissons(1) + 34 * Poissons(2) + 4 * Rr(1) * Poissons(2) -
                        16 * Rr(2) * Poissons(2) - 20 * Rr(1) * Rr(2) * Poissons(2) + 6 * Rr(2) ^ 2 * Poissons(2) + 24 * Rr(1) * Rr(2) ^ 2 * Poissons(2) _
                        - 48 * Poissons(1) * Poissons(2) + 16 * Rr(2) * Poissons(1) * Poissons(2) - 24 * Poissons(2) ^ 2 +
                        16 * Rr(1) * Rr(2) * Poissons(2) ^ 2 - 24 * Rr(1) * Rr(2) ^ 2 * Poissons(2) ^ 2 + 32 * Poissons(1) * Poissons(2) ^ 2 -
                        20 * Rr(2) * Poissons(3) - 12 * Rr(1) * Rr(2) * Poissons(3) - 4 * Rr(2) ^ 2 * Poissons(3) + 4 * Rr(1) * Rr(2) ^ 2 * Poissons(3) +
                        24 * Rr(2) * Poissons(1) * Poissons(3) + 8 * Rr(2) ^ 2 * Poissons(1) * Poissons(3) + 32 * Rr(2) * Poissons(2) * Poissons(3) +
                        40 * Rr(1) * Rr(2) * Poissons(2) * Poissons(3) - 8 * Rr(2) ^ 2 * Poissons(2) * Poissons(3) -
                        32 * Rr(1) * Rr(2) ^ 2 * Poissons(2) * Poissons(3) - 32 * Rr(2) * Poissons(1) * Poissons(2) * Poissons(3) -
                        32 * Rr(1) * Rr(2) * Poissons(2) ^ 2 * Poissons(3) + 32 * Rr(1) * Rr(2) ^ 2 * Poissons(2) ^ 2 * Poissons(3))) / ((3 + Rr(1) -
                        4 * Poissons(1)) * (3 + Rr(2) - 4 * Poissons(2)) * (-1 - 3 * Rr(1) + 4 * Rr(1) * Poissons(2)) * (-1 - 3 * Rr(2) +
                        4 * Rr(2) * Poissons(3)))
                    Case 1
                        fa = (4 * (-1 + Rr(2)) * (-1 + Poissons(1)) * (HLayer(1) + 2 * HLayer(2) +
                                3 * HLayer(1) * Rr(1) - 2 * HLayer(2) * Rr(1) - 4 * HLayer(2) * Poissons(1) -
                                4 * HLayer(1) * Poissons(2) - 12 * HLayer(2) * Poissons(2) -
                                16 * HLayer(1) * Rr(1) * Poissons(2) + 16 * HLayer(2) * Poissons(1) * Poissons(2) _
                                + 16 * HLayer(1) * Rr(1) * Poissons(2) ^ 2)) / ((3 + Rr(1) - 4 * Poissons(1)) * (3 _
                                + Rr(2) - 4 * Poissons(2)) * (-1 - 3 * Rr(1) + 4 * Rr(1) * Poissons(2)))
                    Case 2
                        fa = (8 * HLayer(2) * (-1 + Rr(2)) * (-1 + Poissons(1)) * (-HLayer(1) - 3 * HLayer(2) - 3 * HLayer(1) * Rr(1) -
                            HLayer(2) * Rr(1) + 4 * HLayer(2) * Poissons(1) + 4 * HLayer(1) * Rr(1) * Poissons(2))) / ((3 + Rr(1) -
                            4 * Poissons(1)) * (3 + Rr(2) - 4 * Poissons(2)) * (-1 - 3 * Rr(1) + 4 * Rr(1) * Poissons(2)))
                End Select
            Case 3
                Select Case deg
                    Case 0
                        fa = (-32 * (-1 + Poissons(1)) * (-1 + Poissons(2)) * (20 - 14 * Rr(3) - 6 * Rr(3) ^ 2 _
                                - 27 * Poissons(1) + 18 * Rr(3) * Poissons(1) + 9 * Rr(3) ^ 2 * Poissons(1) -
                                27 * Poissons(2) + 18 * Rr(3) * Poissons(2) + 9 * Rr(3) ^ 2 * Poissons(2) +
                                36 * Poissons(1) * Poissons(2) - 24 * Rr(3) * Poissons(1) * Poissons(2) -
                                12 * Rr(3) ^ 2 * Poissons(1) * Poissons(2) - 53 * Poissons(3) +
                                20 * Rr(3) * Poissons(3) - 3 * Rr(3) ^ 2 * Poissons(3) +
                                72 * Poissons(1) * Poissons(3) - 24 * Rr(3) * Poissons(1) * Poissons(3) +
                                72 * Poissons(2) * Poissons(3) - 24 * Rr(3) * Poissons(2) * Poissons(3) -
                                96 * Poissons(1) * Poissons(2) * Poissons(3) +
                                32 * Rr(3) * Poissons(1) * Poissons(2) * Poissons(3) + 36 * Poissons(3) ^ 2 -
                                48 * Poissons(1) * Poissons(3) ^ 2 - 48 * Poissons(2) * Poissons(3) ^ 2 +
                                64 * Poissons(1) * Poissons(2) * Poissons(3) ^ 2 + 4 * Rr(3) * (7 - 9 * Poissons(2) _
                                + 2 * (-5 + 6 * Poissons(2)) * Poissons(3) + Rr(3) * (2 - 3 * Poissons(2) +
                                Poissons(1) * (-3 + 4 * Poissons(2)) + Poissons(3)) - Poissons(1) * (-3 +
                                4 * Poissons(2)) * (-3 + 4 * Poissons(3))) * Poissons(4) + Rr(2) * (-((-3 +
                                4 * Poissons(3)) * (2 - 2 * Poissons(3) + Poissons(1) * (-3 +
                                4 * Poissons(3)))) + 2 * Rr(3) * (-1 + Poissons(1) + Poissons(3)) * (-3 +
                                4 * Poissons(3)) * (-1 + 2 * Poissons(4)) - Rr(3) ^ 2 * (Poissons(1) + 4 * (-1 +
                                Poissons(3)) * Poissons(3)) * (-3 + 4 * Poissons(4))) + Rr(1) * (6 + 3 * (-5 +
                                2 * Rr(2)) * Poissons(3) + 4 * (3 - 2 * Rr(2)) * Poissons(3) ^ 2 +
                                Poissons(2) * (-8 + 3 * Rr(2) + 4 * (5 - 4 * Rr(2)) * Poissons(3) + 16 * (-1 +
                                Rr(2)) * Poissons(3) ^ 2) - 2 * Rr(3) * (-((-3 + 4 * Poissons(2)) * (-1 +
                                2 * Poissons(3))) + Rr(2) * (-3 + 4 * Poissons(3)) * (1 - Poissons(2) + (-3 +
                                4 * Poissons(2)) * Poissons(3))) * (-1 + 2 * Poissons(4)) +
                                Rr(3) ^ 2 * (Rr(2) * (-2 + 3 * Poissons(2)) - (1 + 4 * Rr(2)) * (-3 +
                                4 * Poissons(2)) * Poissons(3) + 4 * Rr(2) * (-3 +
                                4 * Poissons(2)) * Poissons(3) ^ 2) * (-3 + 4 * Poissons(4))))) / ((3 + Rr(1) -
                                4 * Poissons(1)) * (3 + Rr(2) - 4 * Poissons(2)) * (-1 + Rr(1) * (-3 +
                                4 * Poissons(2))) * (3 + Rr(3) - 4 * Poissons(3)) * (-1 + Rr(2) * (-3 +
                                4 * Poissons(3))) * (-1 + Rr(3) * (-3 + 4 * Poissons(4))))
                    Case 1
                        fa = (-16 * (-1 + Rr(3)) * (-1 + Poissons(1)) * (-1 +
                                Poissons(2)) * (-(HLayer(2) * (3 + Rr(1) - 4 * Poissons(1)) * (-1 +
                                4 * Poissons(3)) * (-1 + Rr(2) * (-3 + 4 * Poissons(3)))) + HLayer(1) * (-1 +
                                Rr(1) * (-3 + 4 * Poissons(2))) * (-1 + 4 * Poissons(3)) * (-1 + Rr(2) * (-3 +
                                4 * Poissons(3))) + 4 * HLayer(3) * (-2 + 3 * Poissons(2) + (9 + 2 * Rr(2) -
                                12 * Poissons(2)) * Poissons(3) - Poissons(1) * (3 + Rr(2) -
                                4 * Poissons(2)) * (-1 + 4 * Poissons(3)) + Rr(1) * (Rr(2) * (2 -
                                3 * Poissons(2)) + (3 - 2 * Rr(2) + 4 * (-1 +
                                Rr(2)) * Poissons(2)) * Poissons(3))))) / ((3 + Rr(1) - 4 * Poissons(1)) * (3 +
                                Rr(2) - 4 * Poissons(2)) * (-1 + Rr(1) * (-3 + 4 * Poissons(2))) * (3 + Rr(3) -
                                4 * Poissons(3)) * (-1 + Rr(2) * (-3 + 4 * Poissons(3))))
                    Case 2
                        fa = (-32 * HLayer(3) * (-1 + Rr(3)) * (-1 + Poissons(1)) * (-1 +
                                Poissons(2)) * (HLayer(1) * (-1 + Rr(1) * (-3 + 4 * Poissons(2))) * (-1 +
                                Rr(2) * (-3 + 4 * Poissons(3))) - (3 + Rr(1) -
                                4 * Poissons(1)) * (-(HLayer(3) * (3 + Rr(2) - 4 * Poissons(2))) +
                                HLayer(2) * (-1 + Rr(2) * (-3 + 4 * Poissons(3)))))) / ((3 + Rr(1) -
                                4 * Poissons(1)) * (3 + Rr(2) - 4 * Poissons(2)) * (-1 + Rr(1) * (-3 +
                                4 * Poissons(2))) * (3 + Rr(3) - 4 * Poissons(3)) * (-1 + Rr(2) * (-3 +
                                4 * Poissons(3))))
                End Select
            Case 4
                Select Case deg
                    Case 0
                        fa = (64 * (-1 + Poissons(1)) * (-1 + Poissons(2)) * (-1 + Poissons(3)) * (-121 -
                        39 * Rr(3) + 82 * Rr(4) + 30 * Rr(3) * Rr(4) + 39 * Rr(4) ^ 2 + 9 * Rr(3) * Rr(4) ^ 2 +
                        162 * Poissons(1) + 54 * Rr(3) * Poissons(1) - 108 * Rr(4) * Poissons(1) -
                        36 * Rr(3) * Rr(4) * Poissons(1) - 54 * Rr(4) ^ 2 * Poissons(1) -
                        18 * Rr(3) * Rr(4) ^ 2 * Poissons(1) + 162 * Poissons(2) + 54 * Rr(3) * Poissons(2) -
                        108 * Rr(4) * Poissons(2) - 36 * Rr(3) * Rr(4) * Poissons(2) - 54 * Rr(4) ^ 2 * Poissons(2) -
                        18 * Rr(3) * Rr(4) ^ 2 * Poissons(2) - 216 * Poissons(1) * Poissons(2) -
                        72 * Rr(3) * Poissons(1) * Poissons(2) + 144 * Rr(4) * Poissons(1) * Poissons(2) +
                        48 * Rr(3) * Rr(4) * Poissons(1) * Poissons(2) + 72 * Rr(4) ^ 2 * Poissons(1) * Poissons(2) +
                        24 * Rr(3) * Rr(4) ^ 2 * Poissons(1) * Poissons(2) + 162 * Poissons(3) -
                        108 * Rr(4) * Poissons(3) - 54 * Rr(4) ^ 2 * Poissons(3) - 216 * Poissons(1) * Poissons(3) _
                        + 144 * Rr(4) * Poissons(1) * Poissons(3) + 72 * Rr(4) ^ 2 * Poissons(1) * Poissons(3) -
                        216 * Poissons(2) * Poissons(3) + 144 * Rr(4) * Poissons(2) * Poissons(3) +
                        72 * Rr(4) ^ 2 * Poissons(2) * Poissons(3) + 288 * Poissons(1) * Poissons(2) * Poissons(3) _
                        - 192 * Rr(4) * Poissons(1) * Poissons(2) * Poissons(3) -
                        96 * Rr(4) ^ 2 * Poissons(1) * Poissons(2) * Poissons(3) + 322 * Poissons(4) +
                        100 * Rr(3) * Poissons(4) - 112 * Rr(4) * Poissons(4) - 52 * Rr(3) * Rr(4) * Poissons(4) +
                        6 * Rr(4) ^ 2 * Poissons(4) + 24 * Rr(3) * Rr(4) ^ 2 * Poissons(4) -
                        432 * Poissons(1) * Poissons(4) - 144 * Rr(3) * Poissons(1) * Poissons(4) +
                        144 * Rr(4) * Poissons(1) * Poissons(4) + 48 * Rr(3) * Rr(4) * Poissons(1) * Poissons(4) -
                        432 * Poissons(2) * Poissons(4) - 144 * Rr(3) * Poissons(2) * Poissons(4) +
                        144 * Rr(4) * Poissons(2) * Poissons(4) + 48 * Rr(3) * Rr(4) * Poissons(2) * Poissons(4) +
                        576 * Poissons(1) * Poissons(2) * Poissons(4) +
                        192 * Rr(3) * Poissons(1) * Poissons(2) * Poissons(4) -
                        192 * Rr(4) * Poissons(1) * Poissons(2) * Poissons(4) -
                        64 * Rr(3) * Rr(4) * Poissons(1) * Poissons(2) * Poissons(4) -
                        432 * Poissons(3) * Poissons(4) + 144 * Rr(4) * Poissons(3) * Poissons(4) +
                        576 * Poissons(1) * Poissons(3) * Poissons(4) -
                        192 * Rr(4) * Poissons(1) * Poissons(3) * Poissons(4) +
                        576 * Poissons(2) * Poissons(3) * Poissons(4) -
                        192 * Rr(4) * Poissons(2) * Poissons(3) * Poissons(4) -
                        768 * Poissons(1) * Poissons(2) * Poissons(3) * Poissons(4) +
                        256 * Rr(4) * Poissons(1) * Poissons(2) * Poissons(3) * Poissons(4) - 216 * Poissons(4) ^ 2 _
                        - 64 * Rr(3) * Poissons(4) ^ 2 + 16 * Rr(3) * Rr(4) * Poissons(4) ^ 2 -
                        24 * Rr(3) * Rr(4) ^ 2 * Poissons(4) ^ 2 + 288 * Poissons(1) * Poissons(4) ^ 2 +
                        96 * Rr(3) * Poissons(1) * Poissons(4) ^ 2 + 288 * Poissons(2) * Poissons(4) ^ 2 +
                        96 * Rr(3) * Poissons(2) * Poissons(4) ^ 2 -
                        384 * Poissons(1) * Poissons(2) * Poissons(4) ^ 2 -
                        128 * Rr(3) * Poissons(1) * Poissons(2) * Poissons(4) ^ 2 +
                        288 * Poissons(3) * Poissons(4) ^ 2 - 384 * Poissons(1) * Poissons(3) * Poissons(4) ^ 2 -
                        384 * Poissons(2) * Poissons(3) * Poissons(4) ^ 2 +
                        512 * Poissons(1) * Poissons(2) * Poissons(3) * Poissons(4) ^ 2 - 4 * Rr(4) * (41 -
                        54 * Poissons(2) + 18 * Poissons(1) * (-3 + 4 * Poissons(2)) - 54 * Poissons(3) +
                        24 * (Poissons(1) * (3 - 4 * Poissons(2)) + 3 * Poissons(2)) * Poissons(3) -
                        56 * Poissons(4) + 8 * (9 * Poissons(2) + 3 * (3 - 4 * Poissons(2)) * Poissons(3) +
                        Poissons(1) * (-3 + 4 * Poissons(2)) * (-3 + 4 * Poissons(3))) * Poissons(4) +
                        Rr(4) * (13 - 18 * Poissons(3) + 6 * Poissons(2) * (-3 + 4 * Poissons(3)) -
                        2 * Poissons(1) * (-3 + 4 * Poissons(2)) * (-3 + 4 * Poissons(3)) + 2 * Poissons(4)) +
                        Rr(3) * ((-5 + 6 * Poissons(2) - Poissons(1) * (-6 + 8 * Poissons(2)) +
                        2 * Poissons(4)) * (-3 + 4 * Poissons(4)) + Rr(4) * (3 - 6 * Poissons(2) +
                        Poissons(1) * (-6 + 8 * Poissons(2)) - 8 * (-1 +
                        Poissons(4)) * Poissons(4)))) * Poissons(5) + Rr(2) * ((-3 + 4 * Poissons(3)) * (13 -
                        2 * Poissons(1) * (3 - 4 * Poissons(4)) ^ 2 - 34 * Poissons(4) + 24 * Poissons(4) ^ 2 +
                        2 * Rr(4) * (5 - 6 * Poissons(1) + 8 * (-1 + Poissons(1)) * Poissons(4)) * (-1 +
                        2 * Poissons(5)) - Rr(4) ^ 2 * (-1 + 2 * Poissons(1) - 2 * Poissons(4)) * (-3 +
                        4 * Poissons(5))) + Rr(3) * ((-3 + 4 * Poissons(4)) * (3 + Poissons(3) * (2 -
                        8 * Poissons(4)) + Poissons(1) * (-6 + 8 * Poissons(4))) + 2 * Rr(4) * (-3 +
                        4 * Poissons(4)) * (3 - 2 * Poissons(1) - 6 * Poissons(4) + Poissons(3) * (-2 +
                        8 * Poissons(4))) * (-1 + 2 * Poissons(5)) + Rr(4) ^ 2 * (2 * Poissons(1) +
                        Poissons(3) * (-6 - 32 * (-1 + Poissons(4)) * Poissons(4)) + 3 * (1 + 8 * (-1 +
                        Poissons(4)) * Poissons(4))) * (-3 + 4 * Poissons(5)))) + Rr(1) * ((3 -
                        4 * Poissons(2)) * (-13 + 18 * Poissons(3) + 2 * (17 - 24 * Poissons(3)) * Poissons(4) +
                        8 * (-3 + 4 * Poissons(3)) * Poissons(4) ^ 2 - 2 * Rr(4) * (5 - 6 * Poissons(3) + 8 * (-1 +
                        Poissons(3)) * Poissons(4)) * (-1 + 2 * Poissons(5)) + Rr(4) ^ 2 * (-1 + 2 * Poissons(3) _
                        - 2 * Poissons(4)) * (-3 + 4 * Poissons(5)) + Rr(3) * (-3 + 4 * Poissons(4) -
                        2 * Rr(4) * (3 + 2 * Poissons(4) * (-5 + 4 * Poissons(4))) * (-1 + 2 * Poissons(5)) +
                        Rr(4) ^ 2 * (1 + 8 * (-1 + Poissons(4)) * Poissons(4)) * (-3 + 4 * Poissons(5)))) +
                        Rr(2) * (Rr(3) * ((-3 + 4 * Poissons(4)) * (-3 + Poissons(3) * (6 - 24 * Poissons(4)) +
                        16 * Poissons(4) + 2 * Poissons(2) * (-3 + 4 * Poissons(3)) * (-1 + 4 * Poissons(4))) -
                        2 * Rr(4) * (-3 + 4 * Poissons(4)) * (-5 + 6 * Poissons(3) + 6 * (3 -
                        4 * Poissons(3)) * Poissons(4) + 2 * Poissons(2) * (-3 + 4 * Poissons(3)) * (-1 +
                        4 * Poissons(4))) * (-1 + 2 * Poissons(5)) + Rr(4) ^ 2 * (13 + 72 * (-1 +
                        Poissons(4)) * Poissons(4) - 6 * Poissons(3) * (-3 + 4 * Poissons(4)) * (-1 +
                        4 * Poissons(4)) + 2 * Poissons(2) * (-3 + 4 * Poissons(3)) * (-3 + 4 * Poissons(4)) * (-1 _
                        + 4 * Poissons(4))) * (-3 + 4 * Poissons(5))) + (-3 + 4 * Poissons(3)) * (3 +
                        Poissons(2) * (2 - 8 * Poissons(4)) - 6 * Poissons(4) + 8 * Poissons(4) ^ 2 +
                        2 * Rr(4) * (3 - 2 * Poissons(2) + 8 * (-1 + Poissons(2)) * Poissons(4)) * (-1 +
                        2 * Poissons(5)) - Rr(4) ^ 2 * (1 - 6 * Poissons(4) + Poissons(2) * (-2 +
                        8 * Poissons(4))) * (-3 + 4 * Poissons(5))))))) / ((3 + Rr(1) - 4 * Poissons(1)) * (3 +
                        Rr(2) - 4 * Poissons(2)) * (-1 + Rr(1) * (-3 + 4 * Poissons(2))) * (3 + Rr(3) -
                        4 * Poissons(3)) * (-1 + Rr(2) * (-3 + 4 * Poissons(3))) * (3 + Rr(4) -
                        4 * Poissons(4)) * (-1 + Rr(3) * (-3 + 4 * Poissons(4))) * (-1 + Rr(4) * (-3 +
                        4 * Poissons(5))))
                    Case 1
                        fa = (64 * (-1 + Rr(4)) * (-1 + Poissons(1)) * (-1 + Poissons(2)) * (-1 +
                        Poissons(3)) * (9 * HLayer(3) + 26 * HLayer(4) + 3 * HLayer(3) * Rr(1) +
                        6 * HLayer(4) * Rr(1) + 3 * HLayer(3) * Rr(2) + 6 * HLayer(4) * Rr(2) +
                        HLayer(3) * Rr(1) * Rr(2) - 6 * HLayer(4) * Rr(1) * Rr(2) + 27 * HLayer(3) * Rr(3) +
                        6 * HLayer(4) * Rr(3) + 9 * HLayer(3) * Rr(1) * Rr(3) - 6 * HLayer(4) * Rr(1) * Rr(3) +
                        9 * HLayer(3) * Rr(2) * Rr(3) - 6 * HLayer(4) * Rr(2) * Rr(3) +
                        3 * HLayer(3) * Rr(1) * Rr(2) * Rr(3) - 26 * HLayer(4) * Rr(1) * Rr(2) * Rr(3) -
                        12 * HLayer(3) * Poissons(1) - 36 * HLayer(4) * Poissons(1) -
                        4 * HLayer(3) * Rr(2) * Poissons(1) - 12 * HLayer(4) * Rr(2) * Poissons(1) -
                        36 * HLayer(3) * Rr(3) * Poissons(1) - 12 * HLayer(4) * Rr(3) * Poissons(1) -
                        12 * HLayer(3) * Rr(2) * Rr(3) * Poissons(1) - 4 * HLayer(4) * Rr(2) * Rr(3) * Poissons(1) -
                        12 * HLayer(3) * Poissons(2) - 36 * HLayer(4) * Poissons(2) -
                        4 * HLayer(3) * Rr(1) * Poissons(2) - 8 * HLayer(4) * Rr(1) * Poissons(2) +
                        12 * HLayer(4) * Rr(1) * Rr(2) * Poissons(2) - 36 * HLayer(3) * Rr(3) * Poissons(2) -
                        12 * HLayer(4) * Rr(3) * Poissons(2) - 12 * HLayer(3) * Rr(1) * Rr(3) * Poissons(2) +
                        8 * HLayer(4) * Rr(1) * Rr(3) * Poissons(2) +
                        36 * HLayer(4) * Rr(1) * Rr(2) * Rr(3) * Poissons(2) +
                        16 * HLayer(3) * Poissons(1) * Poissons(2) + 48 * HLayer(4) * Poissons(1) * Poissons(2) +
                        48 * HLayer(3) * Rr(3) * Poissons(1) * Poissons(2) +
                        16 * HLayer(4) * Rr(3) * Poissons(1) * Poissons(2) - 36 * HLayer(4) * Poissons(3) -
                        12 * HLayer(4) * Rr(1) * Poissons(3) - 8 * HLayer(4) * Rr(2) * Poissons(3) +
                        8 * HLayer(4) * Rr(1) * Rr(2) * Poissons(3) + 12 * HLayer(4) * Rr(2) * Rr(3) * Poissons(3) +
                        36 * HLayer(4) * Rr(1) * Rr(2) * Rr(3) * Poissons(3) +
                        48 * HLayer(4) * Poissons(1) * Poissons(3) +
                        16 * HLayer(4) * Rr(2) * Poissons(1) * Poissons(3) +
                        48 * HLayer(4) * Poissons(2) * Poissons(3) +
                        16 * HLayer(4) * Rr(1) * Poissons(2) * Poissons(3) -
                        16 * HLayer(4) * Rr(1) * Rr(2) * Poissons(2) * Poissons(3) -
                        48 * HLayer(4) * Rr(1) * Rr(2) * Rr(3) * Poissons(2) * Poissons(3) -
                        64 * HLayer(4) * Poissons(1) * Poissons(2) * Poissons(3) - 4 * (27 * HLayer(4) +
                        HLayer(3) * (1 + 4 * Rr(3)) * (3 + Rr(1) - 4 * Poissons(1)) * (3 + Rr(2) -
                        4 * Poissons(2)) + HLayer(4) * (Rr(2) * (9 - 4 * Poissons(1) * (3 + Rr(3) -
                        4 * Poissons(3)) + 4 * (-3 + Rr(3)) * Poissons(3)) + 4 * (-9 * Poissons(1) + 3 * (-3 +
                        4 * Poissons(1)) * Poissons(2) + Rr(3) * (2 - 3 * Poissons(1) + (-3 +
                        4 * Poissons(1)) * Poissons(2)) - (-3 + 4 * Poissons(1)) * (-3 +
                        4 * Poissons(2)) * Poissons(3)) + Rr(1) * ((-3 + 4 * Poissons(2)) * (-3 +
                        4 * Poissons(3)) + Rr(2) * (3 - 4 * Poissons(3) + 4 * Rr(3) * (-2 + Poissons(2) * (3 -
                        4 * Poissons(3)) + 3 * Poissons(3)))))) * Poissons(4) + 16 * HLayer(3) * Rr(3) * (3 +
                        Rr(1) - 4 * Poissons(1)) * (3 + Rr(2) - 4 * Poissons(2)) * Poissons(4) ^ 2 -
                        HLayer(2) * (3 + Rr(1) - 4 * Poissons(1)) * (-1 + Rr(2) * (-3 + 4 * Poissons(3))) * (-1 +
                        4 * Poissons(4)) * (-1 + Rr(3) * (-3 + 4 * Poissons(4))) + HLayer(1) * (-1 + Rr(1) * (-3 _
                        + 4 * Poissons(2))) * (-1 + Rr(2) * (-3 + 4 * Poissons(3))) * (-1 + 4 * Poissons(4)) * (-1 _
                        + Rr(3) * (-3 + 4 * Poissons(4))))) / ((3 + Rr(1) - 4 * Poissons(1)) * (3 + Rr(2) -
                        4 * Poissons(2)) * (-1 + Rr(1) * (-3 + 4 * Poissons(2))) * (3 + Rr(3) -
                        4 * Poissons(3)) * (-1 + Rr(2) * (-3 + 4 * Poissons(3))) * (3 + Rr(4) -
                        4 * Poissons(4)) * (-1 + Rr(3) * (-3 + 4 * Poissons(4))))
                    Case 2
                        fa = (128 * HLayer(4) * (-1 + Rr(4)) * (-1 + Poissons(1)) * (-1 + Poissons(2)) * (-1 _
                        + Poissons(3)) * (HLayer(1) * (-1 + Rr(1) * (-3 + 4 * Poissons(2))) * (-1 + Rr(2) * (-3 +
                        4 * Poissons(3))) * (-1 + Rr(3) * (-3 + 4 * Poissons(4))) - (3 + Rr(1) -
                        4 * Poissons(1)) * (HLayer(2) * (-1 + Rr(2) * (-3 + 4 * Poissons(3))) * (-1 + Rr(3) * (-3 +
                        4 * Poissons(4))) - (3 + Rr(2) - 4 * Poissons(2)) * (-(HLayer(4) * (3 + Rr(3) -
                        4 * Poissons(3))) + HLayer(3) * (-1 + Rr(3) * (-3 + 4 * Poissons(4))))))) / ((3 + Rr(1) _
                        - 4 * Poissons(1)) * (3 + Rr(2) - 4 * Poissons(2)) * (-1 + Rr(1) * (-3 +
                        4 * Poissons(2))) * (3 + Rr(3) - 4 * Poissons(3)) * (-1 + Rr(2) * (-3 +
                        4 * Poissons(3))) * (3 + Rr(4) - 4 * Poissons(4)) * (-1 + Rr(3) * (-3 +
                        4 * Poissons(4))))
                End Select
        End Select
faEnd:
    ...
    Private Function fb(ByRef EvalLayer As Integer, ByRef deg As Integer) As Double ' Kairat
        ...
        Select Case EvalLayer
            ...
            Case 2
                Select Case deg
                    Case 0
                        fb = (-4 * (-1 + Poissons(1)) * (-1 + Rr(1) + 2 * Poissons(1) + 6 * Poissons(2) -
                             8 * Poissons(1) * Poissons(2))) / ((3 + Rr(1) - 4 * Poissons(1)) * (-1 - 3 * Rr(1) + 4 * Rr(1) * Poissons(2)))
                    Case 1
                        fb = (4 * HLayer(1) * (-1 + Poissons(1))) / (3 + Rr(1) - 4 * Poissons(1))
                End Select

            Case 3
                Select Case deg
                    Case 0
                        fb = (-32 * (-1 + Poissons(1)) * (-1 + Poissons(2)) * (-2 + 3 * Poissons(2) + (9 _
                                + 2 * Rr(2) - 12 * Poissons(2)) * Poissons(3) - Poissons(1) * (3 + Rr(2) -
                                4 * Poissons(2)) * (-1 + 4 * Poissons(3)) + Rr(1) * (Rr(2) * (2 -
                                3 * Poissons(2)) + (3 - 2 * Rr(2) + 4 * (-1 +
                                Rr(2)) * Poissons(2)) * Poissons(3)))) / ((3 + Rr(1) - 4 * Poissons(1)) * (3 +
                                Rr(2) - 4 * Poissons(2)) * (-1 + Rr(1) * (-3 + 4 * Poissons(2))) * (-1 +
                                Rr(2) * (-3 + 4 * Poissons(3))))
                    Case 1
                        fb = (-16 * (-1 + Poissons(1)) * (-1 + Poissons(2)) * (-(HLayer(2) * (3 + Rr(1) -
                                4 * Poissons(1))) + HLayer(1) * (-1 + Rr(1) * (-3 + 4 * Poissons(2))))) / ((3 +
                                Rr(1) - 4 * Poissons(1)) * (3 + Rr(2) - 4 * Poissons(2)) * (-1 + Rr(1) * (-3 +
                                4 * Poissons(2))))
                End Select
            ...
            Case 4
                Select Case deg
                    Case 0
                        fb = (64 * (-1 + Poissons(1)) * (-1 + Poissons(2)) * (-1 + Poissons(3)) * (13 +
                        3 * Rr(3) - 18 * Poissons(1) - 6 * Rr(3) * Poissons(1) - 18 * Poissons(2) -
                        6 * Rr(3) * Poissons(2) + 24 * Poissons(1) * Poissons(2) +
                        8 * Rr(3) * Poissons(1) * Poissons(2) - 18 * Poissons(3) + 24 * Poissons(1) * Poissons(3) _
                        + 24 * Poissons(2) * Poissons(3) - 32 * Poissons(1) * Poissons(2) * Poissons(3) +
                        2 * (4 * Rr(3) * (-2 + Poissons(1) * (3 - 4 * Poissons(2)) + 3 * Poissons(2)) + (-3 +
                        4 * Poissons(1)) * (-3 + 4 * Poissons(2)) * (-3 + 4 * Poissons(3))) * Poissons(4) +
                        Rr(2) * (Rr(3) * (-3 + Poissons(3) * (6 - 8 * Poissons(4)) + Poissons(1) * (-2 +
                        8 * Poissons(4))) - (-3 + 4 * Poissons(3)) * (1 - 6 * Poissons(4) + Poissons(1) * (-2 +
                        8 * Poissons(4)))) + Rr(1) * ((-3 + 4 * Poissons(2)) * (-1 + Rr(3) + Poissons(3) * (2 -
                        8 * Poissons(4)) + 6 * Poissons(4)) + Rr(2) * ((3 - 4 * Poissons(3)) * (-1 +
                        2 * Poissons(2) - 2 * Poissons(4)) + Rr(3) * (-13 + 18 * Poissons(3) + 8 * (2 -
                        3 * Poissons(3)) * Poissons(4) + 2 * Poissons(2) * (-3 + 4 * Poissons(3)) * (-3 +
                        4 * Poissons(4))))))) / ((3 + Rr(1) - 4 * Poissons(1)) * (3 + Rr(2) -
                        4 * Poissons(2)) * (-1 + Rr(1) * (-3 + 4 * Poissons(2))) * (3 + Rr(3) -
                        4 * Poissons(3)) * (-1 + Rr(2) * (-3 + 4 * Poissons(3))) * (-1 + Rr(3) * (-3 +
                        4 * Poissons(4))))
                    Case 1
                        fb = (64 * (-1 + Poissons(1)) * (-1 + Poissons(2)) * (-1 +
                            Poissons(3)) * (HLayer(1) * (-1 + Rr(1) * (-3 + 4 * Poissons(2))) * (-1 + Rr(2) * (-3 +
                            4 * Poissons(3))) - (3 + Rr(1) - 4 * Poissons(1)) * (-(HLayer(3) * (3 + Rr(2) -
                            4 * Poissons(2))) + HLayer(2) * (-1 + Rr(2) * (-3 + 4 * Poissons(3)))))) / ((3 + Rr(1) _
                            - 4 * Poissons(1)) * (3 + Rr(2) - 4 * Poissons(2)) * (-1 + Rr(1) * (-3 +
                            4 * Poissons(2))) * (3 + Rr(3) - 4 * Poissons(3)) * (-1 + Rr(2) * (-3 +
                            4 * Poissons(3))))
                ...
                '    Select Case deg
                '        Case 0
                '            fb = (-512 * (-1 + Poissons(1)) * (-1 + Poissons(2)) * (-1 + Poissons(3)) * (-1 + _
                '                Poissons(4)) * (-20 - 6 * Rr(2) - 6 * Rr(3) - 6 * Rr(4) + 6 * Rr(2) * Rr(3) * Rr(4) _
                '                + 27 * Poissons(1) + 9 * Rr(2) * Poissons(1) + 9 * Rr(3) * Poissons(1) + _
                '                3 * Rr(2) * Rr(3) * Poissons(1) + 9 * Rr(4) * Poissons(1) + _
                '                3 * Rr(2) * Rr(4) * Poissons(1) + 3 * Rr(3) * Rr(4) * Poissons(1) + _
                '                Rr(2) * Rr(3) * Rr(4) * Poissons(1) + 27 * Poissons(2) + 9 * Rr(3) * Poissons(2) _
                '                + 9 * Rr(4) * Poissons(2) + 3 * Rr(3) * Rr(4) * Poissons(2) - _
                '                36 * Poissons(1) * Poissons(2) - 12 * Rr(3) * Poissons(1) * Poissons(2) - _
                '                12 * Rr(4) * Poissons(1) * Poissons(2) - _
                '                4 * Rr(3) * Rr(4) * Poissons(1) * Poissons(2) + 27 * Poissons(3) + _
                '                8 * Rr(2) * Poissons(3) - 3 * Rr(2) * Rr(3) * Poissons(3) + 9 * Rr(4) * Poissons(3) _
                '                - 9 * Rr(2) * Rr(3) * Rr(4) * Poissons(3) - 36 * Poissons(1) * Poissons(3) - _
                '                12 * Rr(2) * Poissons(1) * Poissons(3) - 12 * Rr(4) * Poissons(1) * Poissons(3) - _
                '                4 * Rr(2) * Rr(4) * Poissons(1) * Poissons(3) - 36 * Poissons(2) * Poissons(3) - _
                '                12 * Rr(4) * Poissons(2) * Poissons(3) + _
                '                48 * Poissons(1) * Poissons(2) * Poissons(3) + _
                '                16 * Rr(4) * Poissons(1) * Poissons(2) * Poissons(3) + 27 * Poissons(4) + _
                '                9 * Rr(2) * Poissons(4) + 8 * Rr(3) * Poissons(4) - 3 * Rr(3) * Rr(4) * Poissons(4) _
                '                - 9 * Rr(2) * Rr(3) * Rr(4) * Poissons(4) - 36 * Poissons(1) * Poissons(4) - _
                '                12 * Rr(2) * Poissons(1) * Poissons(4) - 12 * Rr(3) * Poissons(1) * Poissons(4) - _
                '                4 * Rr(2) * Rr(3) * Poissons(1) * Poissons(4) - 36 * Poissons(2) * Poissons(4) - _
                '                12 * Rr(3) * Poissons(2) * Poissons(4) + _
                '                48 * Poissons(1) * Poissons(2) * Poissons(4) + _
                '                16 * Rr(3) * Poissons(1) * Poissons(2) * Poissons(4) - _
                '                36 * Poissons(3) * Poissons(4) - 12 * Rr(2) * Poissons(3) * Poissons(4) + _
                '                4 * Rr(2) * Rr(3) * Poissons(3) * Poissons(4) + _
                '                12 * Rr(2) * Rr(3) * Rr(4) * Poissons(3) * Poissons(4) + _
                '                48 * Poissons(1) * Poissons(3) * Poissons(4) + _
                '                16 * Rr(2) * Poissons(1) * Poissons(3) * Poissons(4) + _
                '                48 * Poissons(2) * Poissons(3) * Poissons(4) - _
                '                64 * Poissons(1) * Poissons(2) * Poissons(3) * Poissons(4) + 81 * Poissons(5) + _
                '                (2 * (72 * Poissons(2) * Poissons(3) - 54 * (Poissons(2) + Poissons(3)) - _
                '                6 * Poissons(1) * (-3 + 4 * Poissons(2)) * (-3 + 4 * Poissons(3)) + Rr(4) * (13 - _
                '                18 * Poissons(2) + 6 * (-3 + 4 * Poissons(2)) * Poissons(3) - _
                '                2 * Poissons(1) * (-3 + 4 * Poissons(2)) * (-3 + 4 * Poissons(3))) + 2 * (-3 + _
                '                4 * Poissons(1)) * (-3 + 4 * Poissons(2)) * (-3 + 4 * Poissons(3)) * Poissons(4)) _
                '                + Rr(3) * (2 * Rr(4) * (3 - 6 * Poissons(2) + Poissons(1) * (-6 + _
                '                8 * Poissons(2)) + 2 * Poissons(4)) - (-3 + 4 * Poissons(1)) * (-3 + _
                '                4 * Poissons(2)) * (-3 + 4 * Poissons(4))) + Rr(2) * ((-3 + _
                '                4 * Poissons(3)) * (Rr(4) * (-2 + 4 * Poissons(1)) - (-3 + 4 * Poissons(1)) * (-3 _
                '                + 4 * Poissons(4))) + Rr(3) * ((-3 + 4 * Poissons(1)) * (-3 + 4 * Poissons(4)) _
                '                - 2 * Rr(4) * (3 + 2 * Poissons(1) - 6 * Poissons(4) + Poissons(3) * (-6 + _
                '                8 * Poissons(4)))))) * Poissons(5) + Rr(1) * ((-3 + 4 * Poissons(2)) * (2 - _
                '                3 * Poissons(4) + (-9 - 2 * Rr(4) + 12 * Poissons(4)) * Poissons(5) + _
                '                Poissons(3) * (3 + Rr(4) - 4 * Poissons(4)) * (-1 + 4 * Poissons(5)) + _
                '                Rr(3) * (Rr(4) * (-2 + 3 * Poissons(4)) + (-3 + Rr(4) * (2 - 4 * Poissons(4)) + _
                '                4 * Poissons(4)) * Poissons(5))) + Rr(2) * ((3 - _
                '                4 * Poissons(3)) * (-Poissons(2) + Poissons(4) + 3 * Poissons(5) - _
                '                4 * Poissons(4) * Poissons(5) + Rr(4) * (2 - 2 * Poissons(5) + _
                '                Poissons(2) * (-3 + 4 * Poissons(5)))) + Rr(3) * ((3 - 4 * Poissons(4)) * (2 - _
                '                3 * Poissons(3) + Poissons(2) * (-3 + 4 * Poissons(3)) + Poissons(5)) + _
                '                Rr(4) * (20 - 26 * Poissons(5) + 9 * Poissons(4) * (-3 + 4 * Poissons(5)) - _
                '                3 * Poissons(3) * (-3 + 4 * Poissons(4)) * (-3 + 4 * Poissons(5)) + _
                '                Poissons(2) * (-3 + 4 * Poissons(3)) * (-3 + 4 * Poissons(4)) * (-3 + _
                '                4 * Poissons(5)))))))) / ((3 + Rr(1) - 4 * Poissons(1)) * (3 + Rr(2) - _
                '                4 * Poissons(2)) * (-1 + Rr(1) * (-3 + 4 * Poissons(2))) * (3 + Rr(3) - _
                '                4 * Poissons(3)) * (-1 + Rr(2) * (-3 + 4 * Poissons(3))) * (3 + Rr(4) - _
                '                4 * Poissons(4)) * (-1 + Rr(3) * (-3 + 4 * Poissons(4))) * (-1 + Rr(4) * (-3 + _
                '                4 * Poissons(5))))
                '        Case 1
                '            fb = (-256 * (-1 + Poissons(1)) * (-1 + Poissons(2)) * (-1 + Poissons(3)) * (-1 + _
                '                    Poissons(4)) * (HLayer(1) * (-1 + Rr(1) * (-3 + 4 * Poissons(2))) * (-1 + _
                '                    Rr(2) * (-3 + 4 * Poissons(3))) * (-1 + Rr(3) * (-3 + 4 * Poissons(4))) - (3 + _
                '                    Rr(1) - 4 * Poissons(1)) * (HLayer(2) * (-1 + Rr(2) * (-3 + _
                '                    4 * Poissons(3))) * (-1 + Rr(3) * (-3 + 4 * Poissons(4))) - (3 + Rr(2) - _
                '                    4 * Poissons(2)) * (-(HLayer(4) * (3 + Rr(3) - 4 * Poissons(3))) + _
                '                    HLayer(3) * (-1 + Rr(3) * (-3 + 4 * Poissons(4))))))) / ((3 + Rr(1) - _
                '                    4 * Poissons(1)) * (3 + Rr(2) - 4 * Poissons(2)) * (-1 + Rr(1) * (-3 + _
                '                    4 * Poissons(2))) * (3 + Rr(3) - 4 * Poissons(3)) * (-1 + Rr(2) * (-3 + _
                '                    4 * Poissons(3))) * (3 + Rr(4) - 4 * Poissons(4)) * (-1 + Rr(3) * (-3 + _
                '                    4 * Poissons(4))))
                '    End Select
        End Select
    ...
    Private Function fc(ByRef EvalLayer As Integer, ByRef deg As Integer) As Double 'Kairat

        fc = 0.0

        If EvalLayer = NLayers Then GoTo fcEnd
        ...
        If deg = 0 Then
            fc = 1
            For I = 1 To EvalLayer - 1
                fc = fc * 4 * (1 - Poissons(I)) / (3 + Rr(I) - 4 * Poissons(I))
            Next
            fc = (Rr(EvalLayer) - 1) / (3 + Rr(EvalLayer) - 4 * Poissons(EvalLayer))
        End If

        If deg = 1 Then
            Select Case EvalLayer
                Case 1
                    fc = (2 * HLayer(1) * (-1 + Rr(1))) / (3 + Rr(1) - 4 * Poissons(1))

                Case 2
                    fc = (-8 * (-1 + Rr(2)) * (-1 + Poissons(1)) * (-HLayer(1) - 3 * HLayer(2) -
                        3 * HLayer(1) * Rr(1) - HLayer(2) * Rr(1) + 4 * HLayer(2) * Poissons(1) +
                        4 * HLayer(1) * Rr(1) * Poissons(2))) / ((3 + Rr(1) - 4 * Poissons(1)) * (3 +
                        Rr(2) - 4 * Poissons(2)) * (-1 - 3 * Rr(1) + 4 * Rr(1) * Poissons(2)))

                Case 3
                    fc = (32 * (-1 + Rr(3)) * (-1 + Poissons(1)) * (-1 + Poissons(2)) * (HLayer(1) * (-1 _
                                    + Rr(1) * (-3 + 4 * Poissons(2))) * (-1 + Rr(2) * (-3 + 4 * Poissons(3))) - (3 _
                                    + Rr(1) - 4 * Poissons(1)) * (-(HLayer(3) * (3 + Rr(2) - 4 * Poissons(2))) +
                                    HLayer(2) * (-1 + Rr(2) * (-3 + 4 * Poissons(3)))))) / ((3 + Rr(1) -
                                    4 * Poissons(1)) * (3 + Rr(2) - 4 * Poissons(2)) * (-1 + Rr(1) * (-3 +
                                    4 * Poissons(2))) * (3 + Rr(3) - 4 * Poissons(3)) * (-1 + Rr(2) * (-3 +
                                    4 * Poissons(3))))
                Case 4
                    fc = (-128 * (-1 + Rr(4)) * (-1 + Poissons(1)) * (-1 + Poissons(2)) * (-1 +
                                    Poissons(3)) * (HLayer(1) * (-1 + Rr(1) * (-3 + 4 * Poissons(2))) * (-1 + Rr(2) * (-3 +
                                    4 * Poissons(3))) * (-1 + Rr(3) * (-3 + 4 * Poissons(4))) - (3 + Rr(1) -
                                    4 * Poissons(1)) * (HLayer(2) * (-1 + Rr(2) * (-3 + 4 * Poissons(3))) * (-1 + Rr(3) * (-3 +
                                    4 * Poissons(4))) - (3 + Rr(2) - 4 * Poissons(2)) * (-(HLayer(4) * (3 + Rr(3) -
                                    ...
                                    - 4 * Poissons(1)) * (3 + Rr(2) - 4 * Poissons(2)) * (-1 + Rr(1) * (-3 +
                                    4 * Poissons(2))) * (3 + Rr(3) - 4 * Poissons(3)) * (-1 + Rr(2) * (-3 +
                                    4 * Poissons(3))) * (3 + Rr(4) - 4 * Poissons(4)) * (-1 + Rr(3) * (-3 + 4 * Poissons(4))))
                Case 5
                    fc = (512 * (-1 + Rr(5)) * (-1 + Poissons(1)) * (-1 + Poissons(2)) * (-1 +
                                    Poissons(3)) * (-1 + Poissons(4)) * (HLayer(1) * (-1 + Rr(1) * (-3 +
                                    4 * Poissons(2))) * (-1 + Rr(2) * (-3 + 4 * Poissons(3))) * (-1 + Rr(3) * (-3 +
                                    4 * Poissons(4))) * (-1 + Rr(4) * (-3 + 4 * Poissons(5))) - (3 + Rr(1) -
                                    4 * Poissons(1)) * (HLayer(2) * (-1 + Rr(2) * (-3 + 4 * Poissons(3))) * (-1 +
                                    Rr(3) * (-3 + 4 * Poissons(4))) * (-1 + Rr(4) * (-3 + 4 * Poissons(5))) - (3 +
                                    Rr(2) - 4 * Poissons(2)) * (HLayer(3) * (-1 + Rr(3) * (-3 +
                                    4 * Poissons(4))) * (-1 + Rr(4) * (-3 + 4 * Poissons(5))) - (3 + Rr(3) -
                                    4 * Poissons(3)) * (-(HLayer(5) * (3 + Rr(4) - 4 * Poissons(4))) +
                                    HLayer(4) * (-1 + Rr(4) * (-3 + 4 * Poissons(5)))))))) / ((3 + Rr(1) -
                                    4 * Poissons(1)) * (3 + Rr(2) - 4 * Poissons(2)) * (-1 + Rr(1) * (-3 +
                                    4 * Poissons(2))) * (3 + Rr(3) - 4 * Poissons(3)) * (-1 + Rr(2) * (-3 +
                                    4 * Poissons(3))) * (3 + Rr(4) - 4 * Poissons(4)) * (-1 + Rr(3) * (-3 +
                                    4 * Poissons(4))) * (3 + Rr(5) - 4 * Poissons(5)) * (-1 + Rr(4) * (-3 +
                                    4 * Poissons(5))))
            End Select
        End If
...
fcEnd:
    ...
    Private Function fd(ByRef EvalLayer As Integer) As Double 'Kairat
        ...
        For I = 1 To EvalLayer - 1
            Rs = (1 + Poissons(I + 1)) * Youngs(I) / (Youngs(I + 1) * (1 + Poissons(I)))
            fd = fd * 4 * (Poissons(I) - 1) / (-1 + Rs * (-3 + 4 * Poissons(I + 1)))
        Next
        ...
        '            fd = (-16 * (-1 + Poissons(1)) * (-1 + Poissons(2))) / ((-1 + Rr(1) * (-3 + 4 * Poissons(2))) * (-1 + Rr(2) * (-3 + 4 * Poissons(3))))

    End Function



    Public Sub ComputeResponse2Asym(ByRef ResponseType As clsLEAF.LEAFoptions, ByRef NACarg As Integer, ByRef LEAAircraft() As clsLEAF.LEAFACParms, ByRef LEAStructure As clsLEAF.LEAFStrParms, ByRef Response(,) As Double, ByRef AllResps(,) As clsLEAF.LEAFAllResponses) ' kairat Asym approach
        ...
        If ResponseType = LEAFoptions.VerticalStrain Then
            ...
            If prod = 1 And EvalLayer < 5 And EvalLayer > 1 Then
                Call IntegrateZStrain2Asym(EvalLayer, ZEval, Response) 'Kairat
            Else
                Call IntegrateZStrain2(EvalLayer, ZEval, Response) ' kairat
            End If
            'end kairat asym
    ...
    Private Sub IntegrateZStrain2Asym(ByRef EvalLayer As Integer, ByRef ZEval As Double, ByRef StrainW(,) As Double) ' Uses asymptotic decomposition of integral - Kairat
